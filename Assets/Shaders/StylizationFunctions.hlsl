...
void SobelEdgeSample_float(float4 UV, float2 TexelSize, float depthSensitivity, float normalSensitivity, float3 viewDir, out float Edge, out float angle)
{
        ...
        for (int i = 0; i < 9; i++)
    {
        ...
        SobelNormalX += normal * sobelXKernel[i];
        SobelNormalY += normal * sobelYKernel[i];
    }
    
    float normalEdge = sqrt(dot(SobelNormalX, SobelNormalX) + dot(SobelNormalY, SobelNormalY));
    normalEdge *= normalSensitivity * normalSensitivity * normalSensitivity;
    
    Edge = max(sqrt(SobelX * SobelX + SobelY * SobelY), normalEdge); //sqrt(SobelNormalX * SobelNormalX + SobelNormalY * SobelNormalY));
