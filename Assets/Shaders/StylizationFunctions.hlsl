...
void SobelEdgeSample_float(float4 UV, float2 TexelSize, float depthSensitivity, float normalSensitivity, float3 viewDir, float steps, out float Edge, out float angle)
{
    float SobelX = 0;
    float SobelY = 0;
    ...
    for (int detail = 1; detail <= steps; detail++)
    {
        stepscale = detail / steps;
        angle = dot(middleNormal, viewDir);
        [unroll]
        for (int i = 0; i < 9; i++)
        {
            float3 normal = SHADERGRAPH_SAMPLE_SCENE_NORMAL(float4(UV.xy + sobelSamplePoints[i] * TexelSize.xy * stepscale, UV.zw));
       
        //float faceingcamera = dot(normal, viewDir);
        
        
            float depth = SHADERGRAPH_SAMPLE_SCENE_DEPTH(float4(UV.xy + sobelSamplePoints[i] * (TexelSize.xy * stepscale) * smoothstep(0.95, 0.99, angle), UV.zw));
